...
1. [定例](#定例)
    - [SDKチームMTG](#SDKチームMTG)
    - [プラットフォーム別MTG議事録](#プラットフォーム別MTG議事録)
...
## 勤怠関係
- 出勤簿申請
  - 月末最終勤務日の退勤打刻後に出勤簿申請も行う
- カレンダーへの予定登録
  - 休み、早退、離席などはカレンダーの予定も入れる
  - サ開全体の`勤怠共有用カレンダー`を利用する
  - https://fancs-dev.slack.com/archives/C2GCVN198/p1672966501243359
- 中抜け開始/終了の打刻
  - オフィス⇔自宅の移動時間などで、一定の時間、業務を停止する場合は`中抜けの対象`となります
  - `中抜け開始訂正`、`中抜け終了訂正`の申請をお願いします
  - ただし`業務とみなす場合`、出勤簿上で業務上必要である背景が分かる情報を備考欄に追記対応をお願いします
